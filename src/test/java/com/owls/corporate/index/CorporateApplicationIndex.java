package com.owls.corporate.index;
...
public class CorporateApplicationIndex extends SeleniumInit {
	 ...
	 */
	@Test // pass
	public void verifyCreateCorporateApplication() {

		int numOfFailure = 0;
		int logStep = 1;
		
	/*	String dataFileName="wildlifeControllerApplicationData";
		String configFileName="wildlifeControllerApplicationConfig";
		
		Common.logcase("Testcase Id: TC_013");
		...
		*/
		...
		Common.logstep("Step " + (logStep++)+" Complete Operating Address Details.");
		corporateapplicationIndexPage.completeOperatingDetails(dataFileName);
		
		Common.logstep("Step " + (logStep++)+" Select 'Same as above' checkbox'.");
		applicationIndexPage.checkboxSameasAbove();
		
		Common.logstep("Step " + (logStep++)+" Enter Valid ABN number.");
		corporateapplicationIndexPage.abnNumber();
	
		Common.logstep("Step " + (logStep++)+" Enter Trading Name.");
		corporateapplicationIndexPage.tradingName();
	
		Common.logstep("Step " + (logStep++)+" Enter Primary Business Telephone.");
		corporateapplicationIndexPage.primaryBusinessTelephone(dataFileName);
		
		Common.logstep("Step " + (logStep++)+" Enter Other Business Telephone.");
		corporateapplicationIndexPage.otherBusinessTelephone(dataFileName);
	
		Common.logstep("Step " + (logStep++)+" Enter Corporate Email Address.");
		corporateapplicationIndexPage.corporateEmailAddress(dataFileName);
	
		Common.logstep("Step " + (logStep++)+" Enter Business Website.");
		corporateapplicationIndexPage.website(dataFileName);
	
		Common.logstep("Step " + (logStep++)+" Enter Emplyee's First Name.");
		corporateapplicationIndexPage.firstName(dataFileName);
	
		Common.logstep("Step " + (logStep++)+" Enter Emplyee's Last Name.");
		corporateapplicationIndexPage.lastName(dataFileName);
		
		Common.logstep("Step " + (logStep++)+" Enter Emplyee's Date of Birth.");
		corporateapplicationIndexPage.dateOfBirth(dataFileName);
		
		Common.logstep("Step " + (logStep++)+" Enter Private Telephone Number.");
		corporateapplicationIndexPage.privateTelephone(dataFileName);
		
		Common.logstep("Step " + (logStep++)+" Enter Start Date of Employee.");
		corporateapplicationIndexPage.startDate(dataFileName);
		
		Common.logstep("Step " + (logStep++)+" Enter Role/Position of Employee.");
		corporateapplicationIndexPage.roleofEmployee(dataFileName);
		
		Common.logstep("Step " + (logStep++)+" Enter Street Address.");
		corporateapplicationIndexPage.enterStreetAddress(dataFileName);		
		
		Common.logstep("Step " + (logStep++)+" Enter Business Email.");
		corporateapplicationIndexPage.businessEmail(dataFileName);		
		
		Common.logstep("Step " + (logStep++)+" Click on Add to List.");
		corporateapplicationIndexPage.clickOnAddtoList();
		
		Common.logOptionalAssert(" Verify that Employee Details persisted in Employee Table");
		if(corporateapplicationVerificatioPage.verifyEmployeeRecord())
		{
		...
		Common.logstep("Step " + (logStep++)+" Choose 'YES, please share my business contact details with the public.' radio button.");
		corporateapplicationIndexPage.chooseYes();
	
		Common.logstep("Step " + (logStep++)+" Choose  multiple options from 'wildlife control service you intend to provide'.");
		corporateapplicationIndexPage.controlService();
	
		Common.logstep("Step " + (logStep++)+" Choose Prefered regions.");
		corporateapplicationIndexPage.randomRegion();
	
		Common.logstep((logStep++)+" Upload necessary files.");	
		corporateapplicationIndexPage.UploadFiles();
		...
		@Parameters ({ "CorporateCreateApplication" })
		@Test // pass
		public void verifyCreateApplicationBodyCorporate() {
		...
		/*	Common.logcase("Testcase Id: TC_013");
			...
			*/
			...
			Common.logstep("Step " + (logStep++)+" Click on Corporate Menu.");
			corporateapplicationIndexPage.clickOnCorporate();
			
			Common.logstep("Step " + (logStep++)+" Select any Body Corporate Available.");
			corporateapplicationIndexPage.selectBodyCorporate(dataFileName);
						
			Common.logstep("Step " + (logStep++)+" Click on Application Menu available inside Corporate Name Options.");
			corporateapplicationIndexPage.clickonCorporateApplication();
		...
		/*	Common.logstep("Step " + (logStep++)+" Enter Valid ABN number.");
			...
			/**
			 * Pending now for testing purpose.
			 */
			/*if(! applicationType.contains("Wildlife Controller Licence"))
			{
				Common.logstep("Step " + (logStep++)+" Choose Yes option for 'Are you taking over an existing business that has wildlife in stock?'.");
				corporateapplicationIndexPage.chooseYesOption();
			...
			*/
			...
			Common.logstep("Step " + (logStep++)+" Choose 'Is this employee Nominated Responsible person'.");
			corporateapplicationIndexPage.chooseNominatedOption();	
						
			Common.logstep("Step " + (logStep++)+" Click on Add to List.");
			corporateapplicationIndexPage.clickOnAddtoList();
			
			/*
			 * Need to complete this verification.
			 * */
			Common.logOptionalAssert(" Verify that Employee Details persisted in Employee Table");
			if(corporateapplicationVerificatioPage.verifyEmployeeRecord())
			{
			...
			Common.logMandetoryAssert(" Verify that Created Application "+applicationType+" is available under Corporate Application with 'In Progress' status.");
			if (applicationVerificatioPage.VerifyCreatedApplicationStatus(configFileName,applicationType)) {
				Common.writeProperties(configFileName,"Application_Created_Sucessfully","True","");
	...
	/*	
		...
		@Parameters ({ "CorporatePaymentNotice" })
		@Test // pass
		public void verifyCorporateLicencePaymentNotice(String testCase) {
			...
			try
		     {
		        ...
		        System.err.println(" -----------------------------------------------------------");
		        System.err.println(" -----------------------------------------------------------"+paymentNotice.getName());
		        System.err.println(" -----------------------------------------------------------");
