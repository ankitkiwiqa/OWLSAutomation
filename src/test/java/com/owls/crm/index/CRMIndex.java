...
public class CRMIndex extends SeleniumInit {
	 ...
	 */
	@Parameters ({ "ReviewValidation" })
	@Test(alwaysRun=true)
	public void validateReviewAppliactionTask(String testCase) {
	...
	*/
	...
	@Parameters ({ "CompleteReview" })
	@Test(alwaysRun=true)
	public void completeReviewApplicationTask(String testCase) {
...
*/
	...
	@Parameters ({ "InterviewSiteInspection" })
	@Test(alwaysRun=true)
	 public void completeInterviewInspectionTask(String testName) {
...
*/
		  ...
		  crmpage.advanceSearchForApplicationTask();
			...
			Common.logMandetoryAssert("Verify that Task status should be 'Interview/Site Inspection' and Queue should be "+queueName+".");
	...
	@Parameters ({ "DelegateApproval" })
	 @Test(alwaysRun=true)
	 public void verifyAndCompleteApproveDelegateTask(String testCase) {
	...
	*/	
		  Common.logcase(testCase);
			...
			try {
				String interview_inspection="False";
				String review_additional ="False";
				
				try {
					 interview_inspection = Common.readProperties(configFileName,"Interview_Inspection_Task_Complete");
					...
					// TODO: handle exception
					 review_additional = Common.readProperties(configFileName,"Additional_Review_Condition_Complete");
						if( review_additional.contains("False"))
						{
							throw new SkipException("Skipping Test case as Interview / Site Inspection task is not completed successfully.");
	 ...
	 @Parameters ({ "AwaitingPayment" })
	 @Test(alwaysRun=true)
	 public void completeAwaitingPaymentConfirmationTask(String testCase) {
...
*/
	 ...
	 {	
		 Common.logstep("Step 1. Open CRM Application ");	
		 driver.get(TestData.getCRMURL());
	
		  Common.logstep("Step 2. Choose 'Standard Authentication' login method from Drop down.");
		  crmpage.chooseLoginMethod();
		  Common.pause(5);
		  Common.logstep("Step 3. Login to CRM as admin");
		  crmpage.enterCRMUsername(TestData.getCRMUsername());
		  Common.pause(1);
		  crmpage.enterCRMPassword(TestData.getCRMPassword());
		  Common.pause(1);
		 ...
		 Common.logstep("Step 4. Click on Owls Administrator ");
		 crmpage.clickOnAdministrator();
		 
		 Common.logstep("Step 5. Select Admin option .");
		 crmpage.clickOnAdmin();
		 
		 Common.logstep("Step 6. Click on Test Data CleanUP Option");
		 crmpage.clickOnTestDataCleanUP();
		 
		 String user="Mr. Chirag CHUDASAMA";
		 Common.logstep("Step 7. Select current user: "+user);
		 crmpage.selectUser(user);
		 
		 Common.logstep("Step 9. Click on Run Cleanup ");
		 crmpage.clickRunCleanUp();
		 
	 }
	 
	 	@Parameters ({"InterviewSiteInspectionWithAdditionalCondition"})
		@Test(alwaysRun=true)
		 public void completeInterviewInspectionWithConditionTask(String testCase) throws ParseException {
			...
			*/
			  ...
			  Common.logOptionalAssert("Verify that Inspections record added successfully in 'Inspections' tab.");
			  if(crmverify.verifyInspectionRecord(dataFileName))
			  {
				  Common.optionalAssertPassed();
			  }
			  else
			  ...
			  Common.logstep("Step " + (logStep++) + " Click 'Application Details' tabs.");
			  crmpage.clickOnApplicationDetails();
			  ...
			  Common.logstep("Step " + (logStep++) + " Enter some additional free-text conditions.");
			  crmpage.enterFreeText(dataFileName);
			 
			  Common.logstep("Step " + (logStep++) + " Click on 'Add Freetext Condition' button.");
			  crmpage.clickOnAddButton();
			 
			  Common.logOptionalAssert("Verify that additional free text conditions added successfully.");
			  if(crmverify.verifyTableRowCount())
			  {
				  Common.optionalAssertPassed();
			  }
			  else
			  {
				  Common.optionalAssertFailed();
					Common.makeScreenshot(driver, "AdditionalConditionTable.png");
					numOfFailure++;
					
			  }
			  
			  Common.logstep("Step " + (logStep++) + " Click on close icon to close condition pop-up.");
			  crmpage.clicOnClose();
				
			  Common.logstep("Step " + (logStep++) + " Click on 'Submit To Delegate' button to Move to Next task.");
			  crmpage.clickonSubmiteToDelegate();
			  ...
			  Common.logMandetoryAssert(" Verify that 'Review Additional conditions' task is available in 'Current Task' sub-panel.");
				if (crmverify.verifyCurrentTask("Review Additional conditions")) {
					Common.optionalAssertPassed();
					Common.writeProperties(configFileName,"Interview_Inspection_Additional_Condition_Task_Complete", "True", "");
					Common.logStatus("Pass");
				
				} else {
					Common.optionalAssertPassed();
					Common.writeProperties(configFileName,"Interview_Inspection_Additional_Condition_Task_Complete", "False", "");
					numOfFailure++;
				}
	 	...
	 	@Parameters ({ "ReviewAdditionalTask" })
		 @Test(alwaysRun=true)
		 public void verifyAndCompleteReviewAdditionalTask() {
		...
		*/	  
				...
				try {
					String interview_inspection = Common.readProperties(configFileName,"Interview_Inspection_Additional_Condition_Task_Complete");
					if (interview_inspection.contains("False")) {
						throw new SkipException("Skipping Test case as Interview / Site Inspection with additional condition task is not completed successfully.");
					}
				} catch (Exception e) {
					throw new SkipException("Skipping Test case as Interview / Site Inspection with additional condition  task is not completed successfully.");
				}
			  ...
			  Common.logMandetoryAssert("Verify that Task status of application '"+appdata.getApplicationSubject()+"' should be 'Review Additional conditions' ");
			  if (crmverify.VerifycurrentTaskAndQueue("Review Additional conditions",appdata.getApplicationSubject(),"Wildlife Licensing Queue")) {
					Common.optionalAssertPassed();
			   Common.writeProperties(configFileName,"Review_Additional_Task_Available", "True", "");
			  } else {
				Common.optionalAssertFailed();
			   Common.writeProperties(configFileName,"Review_Additional_Task_Available", "False", "");
			   numOfFailure++;
			  }
				...
				Common.logstep("Step " + (logStep++) + " Click on Start link to Start 'Review Additional conditions' task.");
				crmverify=crmpage.clickOnStart();
			  
			  Common.pause(2);
			  Common.SwitchtoTab(driver,1);
			  
			  Common.logMandetoryAssert("Verify that the user is redirected to Application Details Page.");
				/*if(crmverify.verifyInspectingOfficerAssessmentTabExpanded())
				{
					Common.optionalAssertPassed();
				}
				...
				{
					Common.optionalAssertFailed();
					Common.makeScreenshot(driver, "incorrectReviewApplication.png");
					numOfFailure++;
				}*/
				
				/*Common.logMandetoryAssert("Verify that Comments provided by Inspecting officer is correctly displayed to the Admin User.");
				if(crmverify.verifyInspectionComment(dataFileName))
				{
					Common.optionalAssertPassed();
				}
				...
				Common.logstep("Step " + (logStep++) + " Mousehover on Completion option and choose 'Completed' option to completed Review Application Errors.");
				crmpage.selectApprovalDelegate(appdata.getApplicationSubject());
				...
				if (crmverify.verifyCurrentTask("Approval By Delegate")) {
					...
					Common.optionalAssertPassed();
					Common.writeProperties(configFileName,"Additional_Review_Condition_Complete", "True", "");
					Common.logStatus("Pass");
				} else {
					Common.optionalAssertFailed();
					Common.writeProperties(configFileName,"Additional_Review_Condition_Complete", "False", "");
					numOfFailure++;
				}
